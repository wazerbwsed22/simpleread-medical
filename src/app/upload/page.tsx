...
function SummaryCard({ result }: { result: UploadResult }) {
  ...
  const hasFollowUps =
    ...
    (summary.followUpQuestions.pcp?.length > 0 ||
      summary.followUpQuestions.specialists?.length > 0);
      ...
      <div className="px-5 py-4 flex items-start justify-between gap-4 bg-white border-b border-slate-100">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="font-medium text-slate-900 truncate font-mono text-sm">
              {filename}
            </span>
          </div>
          <div className="flex gap-3 text-xs text-slate-500">
            ...
            {summary.date !== "Not specified" && (
              <span className="flex items-center">{summary.date}</span>
            )}
          </div>
        </div>
        <button
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {summary.keyFindings.map((f, i) => (
                    <li
                      key={i}
                      className="flex gap-3 text-slate-700 items-start"
                    >
                      <span className="text-slate-300 mt-1 shrink-0 text-[10px]">
                        ■
                      </span>
            ...
            <div className="pt-2">
              ...
              <div className="space-y-3">
                {summary.recommendations.map((r, i) => (
                  <div
                    key={i}
                    className="bg-teal-50/50 border border-teal-100 rounded-lg px-4 py-3"
                  >
          ...
          {hasFollowUps && (
            <div className="pt-2">
              ...
              <div className="space-y-4">
                  ...
                  <div className="bg-blue-50/60 border border-blue-100 rounded-lg px-4 py-3">
                    ...
                    <ul className="space-y-2">
                      {summary.followUpQuestions.pcp.map((q, i) => (
                        <li
                          key={i}
                          className="flex gap-2.5 text-slate-700 items-start"
                        >
                          <span className="text-blue-400 font-medium shrink-0 mt-px">
                            ?
                          </span>
                ...
                {summary.followUpQuestions.specialists?.map(
                  (spec, si) =>
                    spec.questions?.length > 0 && (
                      <div
                        key={si}
                        className="bg-purple-50/60 border border-purple-100 rounded-lg px-4 py-3"
                      >
                        <div className="flex items-center gap-2 mb-2.5">
                        ...
                        <ul className="space-y-2">
                          {spec.questions.map((q, qi) => (
                            <li
                              ...
                              className="flex gap-2.5 text-slate-700 items-start"
                            >
                              <span className="text-purple-400 font-medium shrink-0 mt-px">
                                ?
                              </span>
                              <span className="leading-relaxed">{q}</span>
                            </li>
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (event.type === "progress") {
              ...
              setErrors((prev) => [
                ...prev,
                { filename: event.filename, message: event.message },
              ]);
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Add clinical notes, lab results, or imaging reports for analysis.
          </p>
        </div>
          ...
          <>
            ...
            >
              ...
              <p className="text-sm text-slate-500 mb-6">
                PDF, Word, TXT, or Markdown files
              </p>
              ...
              <div className="bg-white border border-slate-200 rounded-lg p-5 space-y-4">
                ...
                <div className="space-y-2">
                  {files.map((f, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between bg-slate-50 border border-slate-100 rounded-md px-4 py-3"
                    >
                ...
                <div className="pt-2">
                  <button
                    ...
                    disabled={uploading}
                    className="w-full bg-teal-600 text-white py-2.5 rounded-md hover:bg-teal-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium text-sm flex items-center justify-center gap-2 shadow-sm cursor-pointer"
                  >
        ...
        {errors.length > 0 && (
          <div className="space-y-3 mt-6">
            {errors.map((e, idx) => (
              <div
                key={idx}
                className="flex items-start gap-3 bg-red-50 border border-red-100 rounded-lg p-4 text-red-800 text-sm"
              >
                <AlertCircle className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                <div>
                  <span className="font-mono font-medium block mb-1">
                    {e.filename}
                  </span>
